...
void create_new_dataset(const std::string, const std::string);
void create_new_dataset(const command_line_parser &);

const std::string NAME = "Data Set";
const std::string COMMENT = "Created by imglab tools.";
// ----------------------------------------------------------------------------------------
    ...
    const std::string folders)
{
    ...
    while ((pos = s.find(delimiter)) != std::string::npos)
    {
        ...
        meta.folderList.push_back(curFold);
        std::vector<file> files = get_files_in_directory_tree(curFold,
                                                              match_endings(".png .PNG .jpeg .JPEG .jpg .JPG .bmp .BMP .dng .DNG .gif .GIF"),
                                                              depth);
        sort(files.begin(), files.end());
...
{
    std::string line;
    std::ifstream configfile("config.txt"); //reading configuration from txt fuke
    int counter = 0;
    ...
    const dlib::image_dataset_metadata::dataset &data)
{
    ...
    cout << "Number of non-ignored boxes: " << num_unignored_boxes << endl
         << endl;
    ...
    try
    {
        ...
        if (parser.option("sort-num-objects"))
        {
            ...
            std::sort(data.images.rbegin(), data.images.rend(),
                      [](const image_dataset_metadata::image &a, const image_dataset_metadata::image &b)
